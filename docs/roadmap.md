...
このドキュメントは、京都サンガF.C.年間スケジュールページと予想スカッド作成ページの開発予定、完了済み機能、次に取り組む候補、保留事項を一箇所に整理するためのロードマップです。
...
運用面では、GitHub Pages確認、本番サーバーへの手動デプロイ、スプレッドシート更新から本番反映までの流れ、UIプロトタイプ運用ルール、CSS棚卸し・確認観点整理に加え、文書管理基準、アーカイブ整理、出典・ライセンス棚卸し、予想スカッドの本番デプロイ前検証を整備済みです。一方で、CSS・JavaScript整理、Googleスプレッドシート運用、年間スケジュール側の実ブラウザ確認自動化は継続確認とします。公式由来画像の利用許諾確認または差し替え判断は保留中です。
...
| 予想スカッド事前検証の運用 | 確認中 | 実行時間と失敗傾向を確認し、必要なら対象分割やキャッシュを検討する。 |
...
7. 追加メモ機能のUIプロトタイプ
8. 求・譲 投稿ツールの本番移植（プロトタイプ確認後。設計は `docs/trade-post-design.md`）
...
| 項目 | 状態 | 理由 |
| --- | --- | --- |
| 公開中のロゴ・背番号加工物 | 保留 | 利用許諾を確認するか、独自表現へ差し替えるか、削除するかの判断を別PRで行う。現時点では新規素材を追加しない。 |
...
1. 完了済み計画とAI作業ログのアーカイブ整理（PR #156で完了）。
2. 公式由来素材、加工物、外部ライブラリの出典・ライセンス棚卸し（PR #157で完了）。
3. 予想スカッドの本番デプロイ前検証（PR #158で完了）。
4. 公開中のロゴと背番号加工物は、利用許諾の確認、独自表現への差し替え、または削除の判断まで保留する。
