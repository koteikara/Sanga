...
### 予想スカッドだけ、バーが16px下がって始まる

`assets/squad.css` の `body` が `padding: 16px 12px 60px` を持っているため、バーがその上余白の
内側から始まります。ほかのページは画面上端からです。

`squad.css` は別セッションが編集中のため、このPRでは触っていません。**次に予想スカッドを触るPRで、
次の2点をまとめて直してください。**

1. `:root{--topbar-gap:16px}` の宣言を消す。トップバーが `position: fixed` だったころに
   `body` の `padding-top` を計算するために置いたもので、固定をやめた今は参照されていません。
2. バーを画面上端から始める。`body` の上余白16pxを外し、そのぶんを中身側へ移します。
   `body { padding: 16px 12px 60px }` を `body { padding: 0 12px 60px }` にして、
   `.proto-title`（`<h1>`）に `margin-top: 16px` を足すのが最小の直し方です。
   見た目の余白は変わりません。

`sanga_slides.html` にも同じずれがありましたが、こちらは更新の終わったページで担当が重ならないため、
このPRで直しました（`body { margin: 20px }` を `margin: 0; padding: 0 20px 20px` にし、
`table` に `margin-top: 10px` を足して `caption` の10pxと合わせています）。
