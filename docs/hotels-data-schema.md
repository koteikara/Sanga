...
- PR #122 時点では公開ページに `public/data/hotel-index.json` の件数と更新情報を表示するプレビュー枠を追加していたが、2026-07-09 の見直しで、日程表と直接関係しない情報を表示しない方針に戻した。
...
`hotel-index.json` は「どの試合にホテルデータがあるか」を示す軽量な一覧であり、将来の存在確認に使う。現時点の公開ページでは、スケジュールと直接関係しないホテル情報は表示しない。
...
- 現在の公開ページは、スケジュールと直接関係しないホテル情報を表示しない。将来表示を追加する場合は、掲載目的、広告・アフィリエイト表記、外部リンク文言、アクセシビリティを別途確認する。
