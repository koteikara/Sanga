...
## 2026-06-23 表示モード・フィルタ・表示列デフォルト値の定数化

### 作業テーマ

`public/assets/app.js` の表示モード・フィルタ・表示列の既定値を、意味が分かる定数として定義する。今回の作業はJavaScript内部の既定値参照の整理であり、仕様変更ではない。

### 定数化するデフォルト値

- `DEFAULT_DISPLAY_MODE`: 従来どおり `card`
- `DEFAULT_FILTER`: 従来どおり `all`
- `DEFAULT_LAYOUT`: 従来どおり `2`

### 変更しない表示モード値

- 有効な表示モード値は従来どおり `card` と `compact` のままにする。
- `mode-card`、`mode-compact`、`display-mode-card`、`display-mode-compact` などCSS連動クラスは変更しない。

### 変更しないフィルタ値

- 有効なフィルタ値は従来どおり `all`、`home`、`away`、`year-2026`、`year-2027`、`tentative`、`marked`、`state-1`、`state-2` のままにする。
- `state-1`、`state-2` とカード状態値 `0`、`1`、`2` の扱いは変更しない。

### 変更しない表示列仕様

- 表示列の有効値は従来どおり `1`、`2`、`3`、`4` のままにする。
- 2列は追加クラスなしの既定表示として扱う仕様を変更しない。
- `layout-1`、`layout-3`、`layout-4` のCSSクラス連動は変更しない。

### 保存形式・復元処理・フィルタ処理に影響がない理由

- LocalStorageキー文字列は変更せず、既存キーをそのまま使う。
- 表示モードとフィルタの保存形式は従来どおりJSON文字列のままにする。
- 表示列の保存形式は従来どおり文字列のままにする。
- 復元時のフォールバック値を同じ文字列の定数参照に置き換えるだけなので、復元結果は変わらない。
- フィルタ判定の分岐、有効値配列、DOM参照、イベントリスナー、処理順は変更しない。
...
- `node tools/validate-matches.js`
- `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict`
- CSSの `{` と `}` の数が一致することを確認する。
- `public/sanga202627season.html` が `assets/style.css` と `assets/app.js` を参照していることを確認する。
- Static Checks相当をローカルで実行し、HTML、CSS、日程JSON、GitHub Actionsワークフローに差分がないことを確認する。
