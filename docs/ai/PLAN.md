...
- `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict` で生成JSONを厳格検証する。
...
- ローカルで `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict` を実行する。
...
- `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict` を実行する。
...
`public/assets/app.js` のLocalStorageキー定数について、JavaScript内部の定数名だけを用途が分かる名前に変更する。これはLocalStorage仕様変更ではなく、既存キー文字列・保存形式・復元処理・削除処理を維持したまま、可読性を上げるための小変更とする。

### 変更対象の定数名

- `key` → `CARD_STATE_STORAGE_KEY`
- `filterKey` → `FILTER_SETTINGS_STORAGE_KEY`
- `displayModeKey` → `DISPLAY_MODE_STORAGE_KEY`
- `layoutKey` → `LAYOUT_STORAGE_KEY`

### 変更しないLocalStorageキー文字列
...
### 保存形式・復元処理・削除処理に影響がない理由

- 変更するのは定数の識別子名だけで、定数に代入する文字列値は変更しない。
- `localStorage.getItem()`、`localStorage.setItem()`、`localStorage.removeItem()` の呼び出し箇所は、参照する定数名だけを置き換え、処理順と引数の意味を変更しない。
- カード状態、フィルタ、表示モード、表示列の保存形式は既存処理のまま維持する。
- 削除ボタンで削除する4キーは、同じキー文字列を持つ新定数を参照するため変更しない。

### 確認方法

- `git diff --check` を実行する。
- `node --check public/assets/app.js` を実行する。
- `node tools/validate-app-contract.js` を実行し、4つのLocalStorageキー文字列が残っていることを確認する。
- `node tools/validate-matches.js` を実行する。
- `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict` を実行する。
- Pythonで `public/assets/style.css` の `{` と `}` の数が一致することを確認する。
- Pythonで `public/sanga202627season.html` が `assets/style.css` と `assets/app.js` を参照していることを確認する。
- `git diff -- public/sanga202627season.html public/assets/style.css public/data/matches.json .github/workflows/static-checks.yml` で変更禁止ファイルに差分がないことを確認する。
- 差分で4つのLocalStorageキー文字列が変更前後で完全一致していることを確認する。

## 2026-06-22 LocalStorage利用可否フラグ名の明確化

### 作業テーマ

JavaScript内部のLocalStorage利用可否フラグ `storageAvailable` を、真偽値かつLocalStorage可否を表すことが分かる `isStorageAvailable` へ改名する。今回はLocalStorage仕様変更ではなく、内部変数名だけの小さな整理とする。

### 変更対象の変数名

- 変更前: `storageAvailable`
- 変更後: `isStorageAvailable`

### 変更しないLocalStorage仕様

- LocalStorageキー文字列は変更しない。
  - `sanga-schedule-button-states-v1`
  - `sanga-schedule-filter-settings-v1`
  - `sanga-schedule-display-mode-v1`
  - `sanga-schedule-layout-v1`
...
- 復元処理は変更しない。
- 削除処理と削除対象キーは変更しない。
- `showStorageUnavailableMessage()` の文言、呼び出しタイミング、関数名は変更しない。
- HTML、CSS、日程データ、Static Checksワークフローは変更しない。

### 保存形式・復元処理・削除処理に影響がない理由

- 変更対象は `public/assets/app.js` 内部の真偽値変数名のみで、LocalStorageキー定数名やキー文字列を変更しないため。
- `true` / `false` の代入タイミング、例外処理、早期return条件を同じ位置のまま維持するため。
- `localStorage.getItem()`、`localStorage.setItem()`、`localStorage.removeItem()` の呼び出し対象と処理順を変更しないため。
- 既存利用者の保存済みデータを読み取るキー文字列とJSON形式を変更しないため。
...
- `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict`
- `public/assets/style.css` の `{` と `}` の数が一致することを確認する。
- `public/sanga202627season.html` が `assets/style.css` と `assets/app.js` を参照していることを確認する。
- Static Checks相当のローカルコマンドが成功することを確認する。
- 4つのLocalStorageキー文字列が変更後も `public/assets/app.js` に残っていることを確認する。
- `public/sanga202627season.html`、`public/assets/style.css`、`public/data/matches.json`、`.github/workflows/static-checks.yml` に差分がないことを確認する。

## 2026-06-22 表示モード・フィルタ関連定数名の明確化

### 作業テーマ

`public/assets/app.js` の表示モード・フィルタ関連定数について、JavaScript内部の定数名だけを定数であることが分かる大文字スネークケースへ変更する。今回は表示モードやフィルタ仕様の変更ではなく、既存値・ラベル文言・保存形式・復元処理・フィルタ処理を維持したまま可読性を上げる小変更とする。
...
### 保存形式・復元処理・フィルタ処理に影響がない理由

- 変更するのは定数の識別子名だけで、配列・オブジェクトの中身は変更しない。
- `readDisplayModeSettings()` と `readFilterSettings()` は参照する定数名だけを置き換え、JSON保存形式やフォールバック値は変更しない。
- `applyScheduleFilter()` の件数表示は参照するラベル定数名だけを置き換え、ラベル文言・条件分岐・処理順は変更しない。
- LocalStorageキー文字列、読み込み、書き込み、削除処理、DOM参照、CSSクラス連動は変更しない。
...
- Pythonで `public/sanga202627season.html` が `assets/style.css` と `assets/app.js` を参照していることを確認する。
- `git diff -- public/sanga202627season.html public/assets/style.css public/data/matches.json .github/workflows/static-checks.yml` で変更禁止ファイルに差分がないことを確認する。
- 差分で表示モード値、フィルタ値、フィルタラベル文言が変更前後で完全一致していることを確認する。

## 2026-06-22 カード状態保持変数名の明確化

### 作業テーマ

`public/assets/app.js` のカード状態保持用内部変数名を、用途が分かる `matchStates` へ改名する。今回の作業はJavaScript内部の変数名整理であり、カード状態仕様やLocalStorage仕様の変更ではない。
...
- カード状態は従来どおり `sanga-schedule-button-states-v1` に、試合IDをキー、状態値 `0` / `1` / `2` を値とするオブジェクトJSONとして保存する。
- フィルタ、表示モード、表示列の保存形式も変更しない。

### 保存・復元・削除・フィルタ処理に影響がない理由

- `readStoredStates()`、`writeStoredStates()`、`removeStorageValue()`、`normalizeMatchState()`、`applyMatchState()`、`syncMatchState()` の関数名・処理順・状態値の扱いは変更しない。
- `CARD_STATE_STORAGE_KEY` の文字列を変更しないため、既存利用者の保存済みカード状態は同じキーから復元される。
- `states[matchId]` 相当の参照先名だけを `matchStates[matchId]` に置き換えるため、保存されるオブジェクト構造は変わらない。
- フィルタ処理は保存済みカード状態を参照する変数名だけが変わり、`state-1` / `state-2` の判定条件は変更しない。
...
- `public/assets/style.css` の `{` と `}` の数が一致することを確認する。
- `public/sanga202627season.html` が `assets/style.css` と `assets/app.js` を参照していることを確認する。
- Static Checks相当のコマンドを実行する。
- LocalStorageキー文字列、状態値 `0` / `1` / `2`、HTML/CSS/日程データ/静的検証ワークフローに意図しない差分がないことを確認する。

## 2026-06-23 共有画像4列表示の横詰まり改善

### 作業テーマ

PR #110 の大会リボン右上端揃えを維持しつつ、共有画像モードの4列表示で日付欄やカード内テキストが横方向に詰まりすぎないように、CSSだけで必要最小限の見た目調整を行う。あわせて、大会リボン左下端の角丸を浅くし、カード本体の角丸と自然に馴染むようにする。
...
- 日付欄幅、日付文字サイズ、カード内余白、節名・対戦相手の文字サイズや行間は `.phone.layout-4.is-screenshot-mode` 配下に限定して調整する。
- 通常表示や1列・2列・3列表示に影響する共通セレクタの値変更は避ける。
- リボン左下端の角丸のみ、全大会リボンの共通形状として `.competition-ribbon` で調整する。

### 変更しないもの

- 日程データ、節番号、対戦相手、会場、キックオフ時刻。
- `public/data/matches.json`。
- JavaScriptロジック、LocalStorageキー、保存形式、表示列・表示モード・フィルタ仕様。
- HTML構造、カードタップ状態仕様、リボンの色・文言・大会種別表示。
- `.github/workflows/static-checks.yml`。

### 大会リボン右上揃えを維持する方針

- `.competition-ribbon` の `top:0` / `right:0` は維持する。
- 右上角のカード端との接続を崩さない。
- `::after` の折り返し表現が左下端の角丸調整と干渉しないか確認する。

### 1列、2列、3列、通常表示への影響を最小化する方針

- 横詰まり対策は `.phone.layout-4.is-screenshot-mode` の専用上書きに絞る。
- 既存の1列、3列、通常表示、コンパクト表示のセレクタ統合や削除は行わない。
- CSSの大規模リファクタリング、並び替え、不要な整形は行わない。
...
前回の `24px` 以上 `75.1px` 以下というクランプでは、衝突回避計算によってフォーメーションごとにカード高が縮小されるため、PR #146 の「全フォーメーションを75.1pxへ統一する」という意図を満たしていなかった。

### 修正方針

- カード高を `UNIFORM_CARD_HEIGHT_PX` 定数で `75.1` に固定する。
- 固定値で最終的に上書きされる衝突回避・ピッチ寸法計算を削除し、実装と仕様を一致させる。
- リサイズ時の再適用経路と既存のカード配置処理は維持する。
- `squad-builder.js` の更新が公開時に反映されるよう、HTMLのJavaScriptバージョンクエリだけを更新する。
- CSS、日程・選手データ、LocalStorage仕様は変更しない。

### 確認方法

- `node --check public/assets/squad-builder.js`
- 全フォーメーション定義を使った静的検証で、カード高の設定値が常に `75.1px` になることを確認する。
- `node tools/validate-players.js`
- `node tools/validate-matches.js`
- `git diff --check`
