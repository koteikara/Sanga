...
`docs/js-inventory.md` と `docs/ai/JS_CHANGE_CHECKLIST.md` と `docs/ai/PLAN.md` に従い、`public/assets/app.js` のLocalStorageキー定数名だけを用途が分かる名前に変更する。LocalStorageキー文字列、保存形式、復元処理、削除処理、処理順、HTML、CSS、日程データは変更しない。Static Checksが成功し、`docs/ai/WORKLOG.md` に変更内容と確認結果を記録できたら完了とする。

## 2026-06-22 LocalStorage利用可否フラグ名の明確化

`docs/js-inventory.md` と `docs/ai/JS_CHANGE_CHECKLIST.md` と `docs/ai/PLAN.md` に従い、`public/assets/app.js` の `storageAvailable` を `isStorageAvailable` に改名する。LocalStorageキー文字列、保存形式、復元処理、削除処理、処理順、HTML、CSS、日程データは変更しない。Static Checksが成功し、`docs/ai/WORKLOG.md` に変更内容と確認結果を記録できたら完了とする。

## 2026-06-22 表示モード・フィルタ関連定数名の明確化

`docs/js-inventory.md` と `docs/ai/JS_CHANGE_CHECKLIST.md` と `docs/ai/PLAN.md` に従い、`public/assets/app.js` の `validDisplayModes`、`validFilters`、`filterLabels` を定数であることが分かる名前へ改名する。表示モード値、フィルタ値、ラベル文言、LocalStorageキー文字列、保存形式、処理順、HTML、CSS、日程データは変更しない。Static Checksが成功し、`docs/ai/WORKLOG.md` に変更内容と確認結果を記録できたら完了とする。

## 2026-06-22 カード状態保持変数名の明確化

「docs/js-inventory.md と docs/ai/JS_CHANGE_CHECKLIST.md と docs/ai/PLAN.md に従い、public/assets/app.js の `states` を `matchStates` に改名する。LocalStorageキー文字列、保存形式、復元処理、削除処理、状態値、フィルタ処理、処理順、HTML、CSS、日程データは変更しない。Static Checksが成功し、docs/ai/WORKLOG.md に変更内容と確認結果を記録できたら完了とする。」

## 2026-06-23 共有画像4列表示の横詰まり改善

PR #110 の大会リボン右上揃えを維持しつつ、共有画像モードの4列表示で日付やカード内テキストが横詰まり・見切れしにくくなるよう、public/assets/style.css を必要最小限で調整する。日程データ、JavaScript、LocalStorageキー、表示列・表示モード・フィルタ仕様は変更しない。Static Checksが成功し、docs/ai/WORKLOG.md に変更内容と確認結果を記録できたら完了とする。

## 2026-07-09 ホテルJSON構造検証の追加

PR #122 のホテル連携スキャフォールドを安全に進めるため、`public/data/hotel-index.json` と将来追加される `public/data/hotels/{match_id}.json` を静的に検証するスクリプトを追加する。現在の空索引は正常扱いし、ホテル詳細JSONが追加された場合は `matches.json` の試合IDとの対応、件数、日付、検索条件、ホテル候補の基本フィールドを確認できたら完了とする。日程データ、HTML、CSS、JavaScript、LocalStorageキー、ホテル実データは変更しない。

## 2026-07-09 ホテルプレビュー表示の削除

スケジュールページ上に表示されているホテル連携プレビューを削除し、公開画面には試合日程表と直接関係しないホテル情報を表示しない。ホテル生成・検証の裏側の足場は残す。日程データ、表示列、表示モード、フィルタ、共有画像、LocalStorageキーを変更せず、静的チェックが成功し、`docs/ai/WORKLOG.md` に変更内容と確認結果を記録できたら完了とする。

## 2026-08-20 PR #146 カード高さ上限の修正

PR #146 相当の変更を確認し、スカッド作成画面のカード高さが衝突回避計算を尊重しながら `24px` 以上 `75.1px` 以下になるよう修正する。日程データ、HTML、CSS、LocalStorage仕様を変更せず、JavaScript構文・選手データ・日程データの静的検証と作業記録を完了条件とする。

## 2026-08-20 PR #146 レビュー指摘対応

PR #146 の本来の意図である「全フォーメーションのカード高さを75.1pxへ統一する」を満たす。フォーメーションごとの衝突回避計算による縮小を残さず、カード高の単一定数を全レイアウトへ適用し、既存の配置・操作・データ仕様を維持する。
