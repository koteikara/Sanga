...
- 大会リボンの `top` / `right` / 幅 / 余白 / 行高を調整し、カード右上に自然に収まるようにした。
- リボン折り返しのサイズと位置を小さくし、主張を控えめにした。
- J1リボンを紫系からネイビー寄りブルーに変更した。
- 天皇杯リボンを濃い金色に変更し、白文字とのコントラストを上げた。
- ルヴァン杯リボンをワイン系のまま少し濃くした。
- カップ戦カードのタイトル右側余白を増やし、タイトルとリボンの干渉を抑えた。
- HOME/AWAY空欄カードの左帯と左背景を白から淡いグレーに変更した。
- CSS変更に合わせて `public/sanga202627season.html` のCSS/JSキャッシュクエリとバージョン表記を更新した。
- 機能、データ構造、LocalStorageキー、フィルタ仕様、画像生成仕様は変更していない。
- `public/data/matches.json` とCSVは変更していない。
...
- `node --check tools/export-matches-to-sheet-csv.js` に成功した。
- `python3 -m http.server 4173 --bind 127.0.0.1` を起動し、`curl -I http://127.0.0.1:4173/public/sanga202627season.html` でHTTP 200を確認した。
- `git diff -- public/data/matches.json docs/sheets/schedule.initial.csv package.json package-lock.json .github/workflows` で変更禁止ファイルに差分がないことを確認した。
...
- 人間の実ブラウザ確認で、2列表示時のリボン位置、天皇杯リボンの視認性、HOME/AWAY空欄帯の自然さ、画像生成時の見た目を確認する。

### 人間が確認すべき点

- `sec01`、`sec03`、`sec39`、`sec40`、`sec45`、`sec46`、`sec47` のカードで、リボン位置・色・左帯が意図どおりに見えること。

## 2026-06-23 大会リボン右上端揃え
...
- 大会リボンの `top` / `right` を `0` にして、カード右上端へ揃えた。
- リボン右上角の角丸をカード角に合わせ、左側の丸みでリボン感を残す形に調整した。
- リボンの折り返し `::after` を右端に接続し、サイズと明度を控えめにした。
- リボン全体の高さ、余白、文字サイズ、最大幅を控えめにした。
- J1リボンだけ最大幅を小さくし、主張を抑えた。
- CSS変更に合わせて `public/sanga202627season.html` のCSSキャッシュクエリとバージョン表記を更新した。
- データ、機能、LocalStorageキー、フィルタ仕様、画像生成仕様、カードタップ仕様、HOME/AWAY判定、大会リボン文言、カード内タイトル変換は変更していない。
- `public/data/matches.json` と `docs/sheets/schedule.initial.csv` は変更していない。
...
- `node --check tools/export-matches-to-sheet-csv.js` に成功した。
- `python3 -m http.server 4173 --bind 127.0.0.1` を起動し、`curl -I http://127.0.0.1:4173/public/sanga202627season.html` でHTTP 200を確認した。
- `git diff -- public/data/matches.json docs/sheets/schedule.initial.csv public/assets/app.js` で変更禁止ファイルに差分がないことを確認した。

### 未確認項目

- 実ブラウザでのPC幅・スマートフォン幅の目視確認、表示列切替、使い方ダイアログ、LocalStorage削除ボタン、画像生成時の見た目は未確認。
- 特に `sec01`、`sec03`、`sec39`、`sec40`、`sec45`、`sec46`、`sec47` のリボンの重なり具合は、人間の実ブラウザ確認が必要。

### 残課題

- 人間の実ブラウザ確認で、右上端に揃えたリボンが日付数字や曜日に大きく重ならず、2列表示と画像生成時にも自然に見えることを確認する。

### 人間が確認すべき点

- J1・天皇杯・ルヴァン杯のリボンがカード右上端に自然に収まり、浮いて見えないこと。
- HOME/AWAY未定カードの淡いグレー帯が維持されていること。

## 2026-06-23 共有画像4列表示の横詰まり改善
...
- 共有画像モード4列表示に限定して、カード左帯を1pxだけ細くし、本文側の横幅をわずかに確保した。
- 共有画像モード4列表示に限定して、カード内本文余白、節名、対戦相手、日付数字の文字サイズ・行間・余白を微調整し、日付やカード内テキストが横詰まり・見切れしにくいようにした。
- 大会リボン左下端の角丸を浅く調整し、カード本体の角丸と馴染む見た目にした。
- 大会リボンの折り返し `::after` を少し小さくし、左下端の角丸調整とぶつかりにくい形にした。
- CSS変更に合わせて `public/sanga202627season.html` のCSSキャッシュクエリとVersion表記を `20260623-5` / `2026.06.23-5` に更新した。
- 日程データ、JavaScript、LocalStorageキー、表示列・表示モード・フィルタ仕様、カードタップ状態仕様は変更していない。

### 変更したCSSセレクタ

- `.competition-ribbon`
- `.competition-ribbon::after`
- `.phone.layout-4.is-screenshot-mode .match-inner`
- `.phone.layout-4.is-screenshot-mode .home .match-inner`
- `.phone.layout-4.is-screenshot-mode .away .match-inner`
- `.phone.layout-4.is-screenshot-mode .ha`
- `.phone.layout-4.is-screenshot-mode .meta`
- `.phone.layout-4.is-screenshot-mode .sec`
- `.phone.layout-4.is-screenshot-mode .date`
- `.phone.layout-4.is-screenshot-mode .date .main`
- `.phone.layout-4.is-screenshot-mode .date .main.compact-date`
- `.phone.layout-4.is-screenshot-mode .team`

### 維持した内容

- 大会リボンの `top:0` / `right:0` は維持し、カード右上端揃えを崩していない。
- J1、ルヴァン杯、天皇杯リボンの色・文言・大会種別表示は変更していない。
- `public/data/matches.json` は変更していない。
- `public/assets/app.js` は変更していない。
- LocalStorageキー、保存形式、表示列・表示モード・フィルタ仕様、カードタップ状態仕様は変更していない。
- 1列、2列、3列表示と通常表示への影響を抑えるため、横詰まり対策は `.phone.layout-4.is-screenshot-mode` 配下に限定した。
...
- `node --check public/assets/app.js` に成功した。
- `node tools/validate-app-contract.js` に成功した。
- CSS波括弧数は `{` / `}` ともに531件で一致した。
- `public/sanga202627season.html` が `assets/style.css` と `assets/app.js` を参照していることを確認した。
- `git diff -- public/data/matches.json public/assets/app.js .github/workflows/static-checks.yml --exit-code` に成功し、変更禁止ファイルに差分がないことを確認した。
- Chromium、Google Chrome、Playwright、Puppeteer の実行コマンドや依存関係が見つからないことを確認したため、この環境では実ブラウザ確認とスクリーンショット取得は未実施。

### 未確認項目

- 実ブラウザでの共有画像モード4列表示の目視確認は未確認。
- 実ブラウザでのJ1、ルヴァン杯、天皇杯リボンの見え方確認は未確認。
- 実ブラウザでのPC幅・スマートフォン幅、1列・2列・3列表示、表示列切替、使い方ダイアログ、LocalStorage削除ボタンは未確認。

### 残課題

- 人間の実ブラウザ確認で、共有画像モード4列表示の日付欄、節名、対戦相手、大会リボン左下端の角丸が自然に見えることを確認する。
- J1、ルヴァン杯、天皇杯リボンで不自然な差が出ていないことを確認する。

### 人間が確認すべき点

- 共有画像モード4列表示で、日付数字が右端で見切れず、節名・対戦相手・大会名が極端に詰まって見えないこと。
- 大会リボンがカード右上端に揃ったまま、左下端の角丸が深くえぐれて見えないこと。
- 1列、2列、3列表示と通常表示に大きな悪化がないこと。

## 2026-06-24 現状整理とドキュメント最新化確認

### 作業テーマ

- 本番デプロイ前後の区切りとして、現在の実装状態・機能状態・ドキュメント状態を棚卸しした。
- 今回は調査、整理、ドキュメント最新化確認のみを対象とし、公開HTML、CSS、JavaScript、日程JSON、GitHub Actionsワークフローの実装修正は行っていない。
...
- 日程データは `public/data/matches.json` の49件で管理されている。
- `public/assets/app.js` が `data/matches.json` を読み込み、試合カード、年見出し、HOME/AWAY、候補日、注記、大会リボンを描画している。
- 表示列は1列、2列、3列、4列を切り替え可能で、2列が既定表示として扱われている。
- 設定パネル内に表示列、表示モード、フィルタ、共有画像生成入口が集約されている。
- 表示モードは通常カードとコンパクトを切り替え可能。
- フィルタは `all`、`home`、`away`、`year-2026`、`year-2027`、`tentative`、`marked`、`state-1`、`state-2` を扱う。
- 試合カード状態、表示列、表示モード、フィルタはLocalStorageに保存され、削除ボタンで初期状態に戻せる。
- 使い方ダイアログと設定パネルはオーバーレイ、閉じるボタン、Escapeキー、フォーカス移動の処理を持つ。
- `modern-screenshot` を使った保存用画像出力と共有画像モードが実装されている。
- J1、天皇杯、ルヴァン杯の大会リボン表示が実装され、カード右上端揃えと左下端の浅い角丸調整が入っている。
- 共有画像モード4列表示の横詰まり改善は `.phone.layout-4.is-screenshot-mode` 配下の限定上書きとして実装されている。
- Static Checksでは日程JSON検証、生成JSON厳格検証、JavaScript構文確認、JavaScript契約チェック、CSS波括弧数、HTMLのCSS/JS参照を確認している。

### 直近で確認した変更内容

- `public/sanga202627season.html` のCSSキャッシュクエリは `assets/style.css?v=20260623-5`、JavaScript参照は `assets/app.js?v=20260623-3` で、Version表記は `2026.06.23-5 / Screenshot 4-column spacing: 2026-06-23 / Data updated: 2026-06-22` だった。
- `public/assets/app.js` 内の `matches.json` 読み込みパスは `data/matches.json?v=20260622-1` だった。
- `.github/workflows/static-checks.yml` の `expected-count` は現行データに合わせて `49` だった。
- `tools/validate-app-contract.js` はStatic Checksに組み込まれており、LocalStorageキー、JSON読み込みパス、DOM/CSSフック、表示列・表示モード・フィルタ有効値を静的に確認している。

### ドキュメント更新が必要だったか

- `docs/css-inventory.md` は、大会リボンと共有画像4列表示の限定上書きについて、既存の一般的な共有画像・4列表示記述だけでは直近調整の注意点が十分に分かりにくかったため、最小限追記した。
- `docs/project-structure.md` は、現在の確認体制にJavaScript契約チェックが含まれることを明示するため、最小限追記した。
- `docs/ai/BROWSER_CHECKLIST.md` は、共有画像4列表示と大会リボンを人間が確認すべき観点として残すため、最小限追記した。
- `docs/js-inventory.md` と `docs/ai/JS_CHANGE_CHECKLIST.md` は、表示列、表示モード、フィルタ、LocalStorage、共有画像、契約チェック、直近の定数名・変数名整理が反映済みだったため、今回は更新不要と判断した。
- `docs/ai/CHECKLIST.md` は汎用確認リストとして大きな不整合はなく、共有画像の詳細確認は `BROWSER_CHECKLIST.md` 側に寄せる方が重複を増やさないため、今回は更新不要と判断した。

### 更新したファイル

- `docs/css-inventory.md`: 大会リボン、共有画像4列表示の限定上書き、確認時の注意点を追記した。
- `docs/project-structure.md`: 自動確認の一覧に `tools/validate-app-contract.js` によるJavaScript契約チェックを追記した。
- `docs/ai/BROWSER_CHECKLIST.md`: 共有画像モード4列表示と大会リボンの実ブラウザ確認観点を追記した。
- `docs/ai/WORKLOG.md`: 今回の棚卸し結果、更新判断、確認結果、未確認項目、残課題を記録した。

### 更新不要と判断したファイルと理由

- `docs/js-inventory.md`: 現行の表示列、表示モード、フィルタ、LocalStorage、共有画像、契約チェック、内部定数名・変数名整理が記録済みだったため。
- `docs/ai/JS_CHANGE_CHECKLIST.md`: `expected-count 49`、JavaScript契約チェック、表示列・表示モード・フィルタ・共有画像関連の確認観点が反映済みだったため。
- `docs/ai/CHECKLIST.md`: 基本確認リストとして大きな不整合はなく、今回追加すべき詳細観点は `docs/ai/BROWSER_CHECKLIST.md` へ追記したため。

### Static Checks / ローカル確認結果

- `git diff --check` に成功した。
- `node tools/validate-matches.js` に成功し、`matches.json` が49件であることを確認した。
- `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict` に成功した。
- `node --check public/assets/app.js` に成功した。
- `node tools/validate-app-contract.js` に成功した。
- CSS波括弧数は `{` / `}` ともに531件で一致した。
- `public/sanga202627season.html` が `assets/style.css` と `assets/app.js` を参照していることを確認した。
- `git diff -- public/sanga202627season.html public/assets/style.css public/assets/app.js public/data/matches.json .github/workflows/static-checks.yml --exit-code` に成功し、実装ファイルとStatic Checksワークフローに意図しない差分がないことを確認した。

### 未確認項目

- 実ブラウザでのPC幅・スマートフォン幅の目視確認は未実施。
- 表示列切替、表示モード切替、フィルタ、使い方ダイアログ、LocalStorage削除ボタンの実ブラウザ操作は未実施。
- 共有画像生成、保存リンク表示、共有画像モード4列表示の実ブラウザ目視確認は未実施。
- 本番公開URLでの反映確認は未実施。

### 残課題

- 本番デプロイ前後に、人間の実ブラウザで `docs/ai/BROWSER_CHECKLIST.md` のスモークチェックを実施する。
- 特に共有画像モード4列表示で、日付数字、節名、対戦相手、大会リボンが大きく見切れていないか確認する。
- J1、天皇杯、ルヴァン杯の大会リボンが通常表示・共有画像モードともにカード右上端に自然に収まるか確認する。

### 次に再開する場合の候補

- 本番デプロイ前の実ブラウザ確認結果を `docs/ai/WORKLOG.md` に追記する。
- 本番デプロイ後、公開URLでCSS/JSキャッシュクエリ、Version表記、日程データ件数、共有画像生成の動作を確認する。
- 共有画像モードや大会リボンに問題があった場合は、CSS本体の変更を別作業として小さく切り出す。

## 2026-06-24 validate-app-contract.js の確認範囲拡張
...
- CSS整理やJavaScript整理の前に、主要なDOMフック、CSS状態クラス、LocalStorageキー、表示列、表示モード、フィルタ、共有画像生成、大会リボン関連の静的契約チェックを拡張した。
- 本番HTML、CSS、JavaScript、公開JSONは参照のみとし、既存機能は変更していない。
...
- `tools/validate-app-contract.js` で `public/assets/style.css` も読み込み、主要CSSフックの存在確認を追加した。
- HTML側の主要DOMフック、data属性、表示列ボタン値、表示モードボタン値、フィルタボタン値、フィルタ表示文言の確認を追加した。
- LocalStorageキー、JSON読み込みパス、表示列、表示モード、フィルタ、カード状態の既存確認を維持した。
- 共有画像生成関連として、`modern-screenshot` / `domToPng`、生成対象、背景色、成功・失敗状態、保存リンク、プレビュー画像、通常表示へ戻る導線の確認を追加した。
- 大会リボン関連として、ラベル生成、class生成、CSSフックの確認を追加した。
- 失敗時に `[app-contract]` 付きで対象契約と不足項目が分かるように出力を調整した。
- `docs/project-structure.md` に契約チェックの確認範囲を追記した。
- `docs/roadmap.md` に、確認範囲拡張を一部実施済みで今後も継続することを反映した。
...
- `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict` に成功した。
- `node --check public/assets/app.js` に成功した。
- `git diff -- public/sanga202627season.html public/assets/app.js public/assets/style.css public/data/matches.json .github/workflows docs/sheets/schedule.initial.csv package.json package-lock.json requirements.txt --exit-code` に成功し、今回変更しない対象に差分がないことを確認した。

### 未確認項目

- 今回は静的チェックツールとドキュメントの更新に限定したため、実ブラウザでの見た目確認は未実施。
- 静的チェックでは、共有画像モードや大会リボンの見た目の正しさまでは保証しない。

### 残課題

- CSS整理・JavaScript整理の小分け作業に合わせて、必要な契約チェックを今後も追加する。
- 共有画像モードや大会リボンの見た目は、`docs/ai/BROWSER_CHECKLIST.md` に沿って実ブラウザで別途確認する。

### 人間が確認すべき点

- 実ブラウザで、共有画像生成、保存導線、通常表示へ戻る導線、大会リボンの見た目に問題がないこと。
- 今後のCSS整理・JavaScript整理で、今回追加した静的契約チェックが過剰に厳密化していないか。

## 2026-06-24 JavaScript整理計画と大会フィルタ追加
...
- `docs/js-inventory.md` に「JavaScript整理の実施計画」を追加し、定数・DOM参照・フィルタ・表示列/表示モード・LocalStorage・描画・共有画像・パネル操作の順で段階的に整理する方針を記録した。
- JavaScript整理対象を低リスク・中リスク・高リスクに分類し、大会フィルタを今後のフィルタ処理整理時に守る契約として記録した。
- 設定パネルのフィルタに `J1`、`天皇杯`、`ルヴァン杯` を追加した。
- `public/assets/app.js` の有効フィルタ値、表示ラベル、判定処理に `competition-j1`、`competition-emp`、`competition-lev` を追加した。
- 大会フィルタは既存JSONの `competition` 値 `J1`、`EMP`、`LEV` に合わせて判定するようにした。
- 使い方パネルに、大会で絞り込めることを追記した。
- `tools/validate-app-contract.js` に大会フィルタのHTML値、表示ラベル、JS有効値、JS判定処理の契約チェックを追加した。
- `docs/filtering.md` に大会フィルタの対象、LocalStorage保存、共有画像生成との関係を追記した。
- `docs/roadmap.md` に、大会フィルタ追加済みであることと、天皇杯・ルヴァン杯の表示/フィルタが一部対応済みであることを追記した。
- `docs/ai/JS_CHANGE_CHECKLIST.md` に、大会フィルタ値の整合確認を追加した。
- `public/data/matches.json`、`.github/workflows/`、`docs/sheets/schedule.initial.csv`、`package.json`、`package-lock.json`、`requirements.txt` は変更していない。

### 確認結果

- `git diff --check` を実行し、空白エラーがないことを確認した。
- `node tools/validate-app-contract.js` を実行し、大会フィルタを含む契約チェックが通ることを確認した。
- `node tools/validate-matches.js` を実行し、公開JSONとHTMLの基本検証が通ることを確認した。
- `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict` を実行し、公開JSONの件数と形式が通ることを確認した。
- `node --check public/assets/app.js` を実行し、JavaScript構文エラーがないことを確認した。
- 変更禁止対象に差分がないことを `git diff -- public/data/matches.json .github/workflows docs/sheets/schedule.initial.csv package.json package-lock.json requirements.txt --exit-code` で確認した。

### 未確認項目

- 実ブラウザでの設定パネル表示、J1/天皇杯/ルヴァン杯フィルタ操作、共有画像生成時の見た目確認は未確認。
- 大会フィルタ適用時に画像生成したPNGの見た目、0件の年見出しが残らないこと、ブラウザコンソールに赤いJavaScriptエラーが出ないことは、人間の実ブラウザ確認が必要。

### 残課題

- JavaScript整理の本格実施は未着手。今回追加した実施計画に沿って、定数・DOM参照・フィルタ処理から小さく進める。
- ACLなど追加大会を扱う場合は、`matches.json` の大会値、表示ラベル、フィルタ値、契約チェック、ドキュメントを同時に更新する必要がある。

### 人間が確認すべき点

- 設定パネルのフィルタに `J1` / `天皇杯` / `ルヴァン杯` が自然に表示されること。
- J1、天皇杯、ルヴァン杯でそれぞれ正しく絞り込めること。
- HOME/AWAY、年、未確定、枠線あり、赤色枠、水色枠の既存フィルタが壊れていないこと。
- 大会フィルタ適用時に共有画像生成へ絞り込み結果が反映されること。
- 使い方パネルの大会フィルタ説明が自然で、スマートフォン幅でも読みにくくないこと。

## 2026-06-24 フィルタ関連処理の整理

### 作業テーマ

PR #116で追加された大会フィルタを含むフィルタ関連処理を、利用者向け挙動を変えずに保守しやすい構造へ整理する。

### 現状調査

- HTMLの `data-filter` は `all`、`home`、`away`、`year-2026`、`year-2027`、`tentative`、`marked`、`state-1`、`state-2`、`competition-j1`、`competition-emp`、`competition-lev` であることを確認した。
- フィルタ表示ラベルは `すべて`、`HOME`、`AWAY`、`2026`、`2027`、`未確定`、`枠線あり`、`赤色枠`、`水色枠`、`J1`、`天皇杯`、`ルヴァン杯` であることを確認した。
- LocalStorageキーは `sanga-schedule-filter-settings-v1`、保存形式は `{"activeFilter":"competition-j1"}` のような既存形式であることを確認した。
- `matches.json` の `competition` 値は `J1`、`EMP`、`LEV` で、大会フィルタ `competition-j1` / `competition-emp` / `competition-lev` と対応していることを確認した。
- 共有画像生成は `shareCaptureTarget` の現在DOM状態を `domToPng` で画像化するため、フィルタ後の `hidden` 状態と年見出し表示状態を維持する必要があることを確認した。
- `tools/validate-app-contract.js` はLocalStorageキー、DOM/CSSフック、フィルタ値・ラベル、大会フィルタ、主要共有画像生成契約を静的チェックしていることを確認した。
...
- `FILTER_DEFINITIONS` を追加し、フィルタ値・表示ラベル・判定種別を一箇所で確認できる構造に整理した。
- 互換契約として `VALID_FILTERS`、`FILTER_LABELS`、`DEFAULT_FILTER`、`activeFilter`、`normalizeFilter()`、`doesCardMatchFilter()`、`applyScheduleFilter()` を維持した。
- 大会フィルタを `type: 'competition'` として他のフィルタと同じ定義構造で扱うようにした。
- `getFilterDefinition()`、`getFilterLabel()`、`getCardCompetition()`、`doesCardMatchDefinition()`、`getScheduleCards()`、`updateCardVisibility()`、`updateFilterButtonStates()` を追加し、判定処理と表示更新処理の責務を分けた。
- `tools/validate-app-contract.js` を新しいフィルタ定義構造に追従させた。
- フィルタ整理内容と今後の確認観点を関連ドキュメントへ反映した。

### 確認結果

- 静的検証コマンドは成功した。
- 変更禁止対象に差分がないことを確認した。
- HTML、CSS、`public/data/matches.json`、`package.json`、`package-lock.json`、`.github/workflows/` は変更していない。
- LocalStorageキーと保存形式は変更していない。
- 共有画像生成処理本体は変更していない。
...
- 実ブラウザでのフィルタ操作確認は未実施。
- 実ブラウザでの共有画像生成確認は未実施。
- GitHub Pages上での表示・操作確認はPRマージ後に実施が必要。

### 残課題

- 表示列、表示モード、LocalStorage全体、共有画像生成、ダイアログ操作のさらなる責務整理は継続候補。
- フィルタを複数条件化する場合は、今回維持した単一選択の保存形式と別タスクで移行方針を検討する。

### 人間が確認すべき点

- GitHub Pagesで既存フィルタと大会フィルタが壊れていないこと。
- 0件時に年見出しだけが残らないこと。
- 大会フィルタ適用時の共有画像生成に現在の絞り込み結果が反映されること。
- ブラウザコンソールに赤いJavaScriptエラーが出ていないこと。

## 2026-06-24 PR #117反映に伴うJSキャッシュ対策
...
PR #117で `public/assets/app.js` を変更したことに伴い、静的サイトで古いJavaScriptキャッシュが使われる可能性を避けるため、HTML側の `app.js?v=` とページ下部バージョン表記を更新する。
...
- `public/sanga202627season.html` の `assets/app.js?v=20260624-1` を `assets/app.js?v=20260624-2` に更新した。
- ページ下部の `.site-version` を `Version 2026.06.24-2 / Filter refactor cache refresh: 2026-06-24 / Data updated: 2026-06-22` に更新した。
- CSSは変更していないため、`assets/style.css?v=20260623-5` は更新していない。
- `public/assets/app.js`、`public/assets/style.css`、`public/data/matches.json`、`tools/validate-app-contract.js` は変更していない。

### 確認結果

- `public/sanga202627season.html` の `app.js?v=` が `20260624-2` へ更新されていることを確認した。
- ページ下部バージョン表記が `Version 2026.06.24-2` へ更新され、`Data updated: 2026-06-22` が維持されていることを確認した。
- CSSクエリを変更していないことを確認した。
- 指定された静的検証コマンドを実行し、結果を確認した。
- 今回変更しない対象に差分がないことを確認した。
- PR本文の見出しが `概要 / 変更内容 / 確認内容 / 注意点` であり、禁止された英語見出しを含まないことをセルフチェックした。

### 未確認項目

- ローカルサーバーは起動確認のみ実施し、実ブラウザでの目視確認とブラウザコンソール確認は未実施。
- GitHub Pages上での表示・操作確認はPRマージ後に実施が必要。

### 残課題

- PRマージ後、GitHub Pagesで新しい `app.js?v=20260624-2` が参照されていることと、既存フィルタ・大会フィルタが壊れていないことを確認する。

### 人間が確認すべき点

- GitHub PagesでHTMLが新しい `app.js?v=20260624-2` を参照していること。
- ページ下部バージョン表記が更新されていること。
- 既存フィルタと大会フィルタが壊れていないこと。
- ブラウザコンソールに赤いJavaScriptエラーが出ていないこと。
...
表示列と表示モードについて、利用者から見える挙動、LocalStorageキー、保存形式、CSS連動を変えずに、定義・保存復元・適用・ボタン状態更新を追いやすい構造へ整理しました。
...
- 表示列値、表示ラベル、`.phone` 用CSSクラス対応を `LAYOUT_DEFINITIONS` にまとめました。
- 表示列の有効値を `VALID_LAYOUTS` として定義から生成し、`normalizeLayout()`、`getLayoutDefinition()`、`updatePhoneLayoutClass()`、`updateLayoutButtonStates()` に責務を分けました。
- `2列` は追加クラスなしの既定状態として維持し、`layout-2` は追加していません。
- 表示モード値、表示ラベル、`.phone` と `[data-json-preview-list]` 用CSSクラス対応を `DISPLAY_MODE_DEFINITIONS` にまとめました。
- 表示モードの有効値を `VALID_DISPLAY_MODES` として定義から生成し、`getDisplayModeDefinition()`、`updatePhoneDisplayModeClass()`、`updateListDisplayModeClass()`、`updateDisplayModeButtonStates()` に責務を分けました。
- `setScheduleLayout()`、`normalizeDisplayMode()`、`applyDisplayMode()` は契約として維持しました。
- `LAYOUT_STORAGE_KEY` と `DISPLAY_MODE_STORAGE_KEY` のキー文字列、表示列の文字列保存、表示モードの `{ "mode": "..." }` 保存形式は変更していません。
- `public/sanga202627season.html` は `app.js?v=` とページ下部バージョン表記のみ更新しました。`Data updated` は変更していません。
- `tools/validate-app-contract.js` に表示列・表示モードの定義構造、ラベル、CSSクラス、契約関数の確認を追従しました。
- `docs/js-inventory.md`、`docs/display-modes.md`、`docs/ai/JS_CHANGE_CHECKLIST.md`、`docs/roadmap.md` に今回の整理内容と今後の確認観点を追記しました。

### 確認結果

- 静的検証で、JavaScript構文、アプリ契約、日程JSON、生成JSON、変更禁止対象の差分なしを確認しました。
- CSS、日程JSON、GitHub Actions、初期CSV、依存ファイルは変更していないことを差分確認しました。
- 共有画像生成処理本体（`generateShareImage()`、`enterScreenshotMode()`、`exitScreenshotMode()`、`shareCaptureTarget`、`domToPng`）は変更していません。
- PR本文の見出しは `概要 / 変更内容 / 確認内容 / 注意点` のみとし、禁止英語見出しを含めない方針でセルフチェックします。

### 未確認項目

- 実ブラウザでの表示列 `1列 / 2列 / 3列 / 4列` 切替、`aria-pressed`、LocalStorage保存・復元の操作確認。
- 実ブラウザでの表示モード `通常カード / コンパクト` 切替、`aria-pressed`、LocalStorage保存・復元の操作確認。
- 保存内容削除で `2列 / 通常カード` へ戻ることの実ブラウザ確認。
- 共有画像生成に現在の表示列・表示モードが反映されることの実ブラウザ確認。
- ブラウザコンソールに赤いJavaScriptエラーが出ていないことの確認。

### 残課題

- LocalStorage処理全体、試合カード描画、共有画像生成、ダイアログ操作の整理は別PRで継続候補です。
- 実ブラウザ確認はGitHub Pagesまたはローカル環境で人間が最終確認する必要があります。

### 人間が確認すべき点

- GitHub Pagesで表示列と表示モードの切替、保存・復元、保存内容削除、フィルタ、カードタップ状態、共有画像生成が壊れていないこと。
- `2列` 選択時に `.phone` へ `layout-2` が付かないこと。
- ページ下部バージョン表記が更新され、`Data updated: 2026-06-22` が維持されていること。

## 2026-06-24 LocalStorage関連処理の整理

### 使用した流れ

- `/plan`: LocalStorageキー一覧、保存・復元・削除、利用不可時、JSONパース失敗時、保存内容削除後の初期化を確認し、小さく整理する方針で進めた。
- `/goal`: 既存キー、保存形式、初期値、削除ボタン文言、CSS、日程JSONを変更しないことを完了条件にした。
- CHECKLIST: `docs/ai/JS_CHANGE_CHECKLIST.md` と指定の静的検証項目に沿って確認した。
- WORKLOG: 本項目に変更内容、確認結果、未確認項目、残課題を記録した。
...
- `STORAGE_KEYS` でLocalStorageキーを一覧化し、既存定数名は維持した。
- `readStorageJson()` / `writeStorageJson()` を追加し、JSON保存・復元処理を状態別処理から分けた。
- 試合カード状態、フィルタ状態、表示列、表示モードの既存キーと保存形式を維持した。
- 保存内容削除処理を `clearAllStoredSettings()` と `resetUiStateAfterStorageClear()` に分け、削除対象キー一覧から削除する形に整理した。
- `validate-app-contract.js` にLocalStorageキー一覧、ヘルパー、状態別保存・復元、削除処理の静的確認を追加した。
- `app.js?v=` とページ下部バージョン表記を更新した。CSSクエリと `Data updated` は変更していない。
...
- `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict` に成功した。
- `node --check public/assets/app.js` に成功した。
- 変更しない対象に差分がないことを `git diff -- public/assets/style.css public/data/matches.json .github/workflows docs/sheets/schedule.initial.csv package.json package-lock.json requirements.txt --exit-code` で確認した。

### 未確認項目

- 実ブラウザでの保存・復元・削除確認は未実施。
- 共有画像生成に現在の保存状態が反映されることの実ブラウザ確認は未実施。
- ブラウザコンソールに赤いJavaScriptエラーが出ていないことの確認は未実施。

### 残課題

- GitHub Pagesまたはローカル実ブラウザで、カードタップ状態、フィルタ、大会フィルタ、表示列、表示モードの保存・復元を確認する。
- 保存内容削除で、カード状態なし / 2列 / 通常カード / すべて表示へ戻ることを確認する。
- 共有画像生成に現在の保存状態が反映されることを確認する。

### 人間が確認すべき点

- GitHub Pagesで保存・復元・削除の挙動が既存どおりか。
- 保存内容削除ボタンの文言、位置、説明文が意図せず変わっていないか。
- 共有画像生成時に、フィルタ、表示列、表示モード、カード状態が反映されているか。
