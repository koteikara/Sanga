...
- `public/assets/app.js` に `DEFAULT_DISPLAY_MODE`、`DEFAULT_FILTER`、`DEFAULT_LAYOUT` を追加した。
- 表示モード、フィルタ、表示列のフォールバックや削除後初期化で使う既定値を定数参照へ置き換えた。
- 既定値は従来どおり `card`、`all`、`2` のままにした。
- 表示モード値、フィルタ値、表示列仕様は変更していない。
- LocalStorageキー文字列、保存形式、復元処理、削除処理は変更していない。
- 処理順、関数名、イベントリスナー、DOM参照、CSSクラス連動は変更していない。
- `docs/js-inventory.md` に既定値定数の記録を追記した。
- `docs/ai/JS_CHANGE_CHECKLIST.md` にデフォルト値変更時の確認観点を最小限追記した。
- `docs/ai/PLAN.md` と `docs/ai/GOAL.md` に今回の作業範囲と完了条件を記録した。
- `public/sanga202627season.html`、`public/assets/style.css`、`public/data/matches.json`、`.github/workflows/static-checks.yml` は変更していない。
...
- `node tools/validate-matches.js` に成功した。
- `node tools/validate-generated-matches.js public/data/matches.json --expected-count 49 --strict` に成功した。
- Python確認で `public/assets/style.css` の `{` と `}` の数が一致することを確認した。
- Python確認で `public/sanga202627season.html` が `assets/style.css` と `assets/app.js` を参照していることを確認した。
- Static Checks相当のローカル確認に成功した。
- `git diff -- public/sanga202627season.html public/assets/style.css public/data/matches.json .github/workflows/static-checks.yml --exit-code` で変更禁止ファイルに差分がないことを確認した。
- `git diff` で表示モード有効値 `card` / `compact`、フィルタ有効値、表示列有効値 `1` / `2` / `3` / `4` は変更されていないことを確認した。

### 未確認項目

- 実ブラウザでのPC幅・スマートフォン幅の目視確認、表示列切替、表示モード切替、フィルタ切替、LocalStorage削除ボタンの操作確認は未実施。
- 今回は定数参照への置き換えのみで表示仕様を変更していないため、スクリーンショット取得は未実施。

### 残課題

- 後続で表示モード・フィルタ・表示列の仕様を整理する場合は、今回追加した定数と `tools/validate-app-contract.js` の契約チェックを基準に、既存利用者の保存状態への影響を確認する。

### 人間が確認すべき点

- 実ブラウザで、初回表示が従来どおりカード表示・全件フィルタ・2列表示になること。
- 保存済み表示設定がある場合に、従来どおり復元されること。
