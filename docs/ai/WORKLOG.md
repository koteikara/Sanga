...
## 2026-09-08 ミニマップを広い画面で広げ、月の目盛りを足す
...
広い画面でシーズン全体を一度に見る手段として、カレンダー本体の多段組ではなくミニマップを
広げる方を採りました（依頼による）。多段組の狙いは「一望」で、それは元からミニマップの役目です。
リストの構造を変えずに済み、多段組で噛み合わなくなるミニマップを隠す必要もありません。

2つです。

1. **幅700px以上でミニマップを26px→76pxにする。** メディアクエリ1つで、狭い端末は26pxのまま
   変わりません。`.stage` は flex なので、その分リストが668px→594pxになります。相手名の省略は
   幅390px以上で0件のままです。
2. **56px以上あるときだけ月の目盛りを出す。** 月初の行と先頭行に「8月」〜「6月」を左端へ重ねます。
   帯の色に直接載せると読めないので、`rgba(251,250,252,.88)` の地を敷いてから字を置きます。
   26pxでは「10月」が入らないので出しません。`minimapRows` に `month` を持たせ、`draw()` で
   今日の線より先に描いて、今日が上に来る順にしています。

### 確認結果

`npm run check` は終了コード0。Chromium（Playwright）で幅320/390/430/768/1280/1920pxを測りました。

| 幅 | ミニマップ | リスト | 横あふれ | 相手名の省略 |
| --- | --- | --- | --- | --- |
| 320px | 26px | 264px | 0px | 2件 |
| 390px | 26px | 334px | 0px | 0件 |
| 430px | 26px | 374px | 0px | 0件 |
| 768px以上 | 76px | 594px | 0px | 0件 |

幅430px以下は変更前と同じ数値です。JSエラーはありません。

キャンバスの画素を直接読み、上端から下端（`#edeae3`）まで塗られていることを確認しました。
要素スクリーンショットの下端に暗い帯が写るのは、ミニマップが画面下（1038px）まで伸びていて
Playwrightが継ぎ足し撮影するときの写り込みで、実物ではありません。

### 未確認項目

- 実機（iPhone Safari・Android Chrome）。狭い端末では26pxのまま何も変えていないので、
  影響を受けるのはタブレットとPCだけです。

### 人間が確認すべき点

- 76pxが広すぎないか。リストが594pxに減るのとの釣り合い。
- 月の目盛りの地（`rgba(251,250,252,.88)`）が、その行の帯の色を隠しすぎないか。
  月初の1行だけなので影響は小さいはずですが、好みの分かれるところです。
