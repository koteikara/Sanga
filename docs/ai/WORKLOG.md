...
## 2026-06-22 スプレッドシートCSVからカップ戦日程を公開JSONへ反映

### 作業テーマ

更新済み `docs/sheets/schedule.initial.csv` を正本として、天皇杯・ルヴァン杯を含む49件の日程データを `public/data/matches.json` へ再生成反映した。

### 変更ファイル

- `docs/sheets/spreadsheet-export-check.md`
- `docs/ai/WORKLOG.md`
- `public/data/matches.json`
- `public/assets/app.js`
- `public/sanga202627season.html`
- `tools/generate-matches-from-csv.js`
- `tools/validate-generated-matches.js`
- `tools/validate-matches.js`

### 変更内容

- `node tools/generate-matches-from-csv.js docs/sheets/schedule.initial.csv public/data/matches.json` で公開JSONを再生成した。
- `public/data/matches.json` を49件に更新し、`sec01`〜`sec49` を含めた。
- カップ戦の勝ち上がり前提行で `ホームアウェイ` / `ホームアウェイ表示` が空欄になるため、生成・検証スクリプトで空欄を許容した。
- JSONプレビュー生成時に `home_away` 空欄をHOME/AWAYへ誤変換しないようにした。
- `public/assets/app.js` のJSON読み込みキャッシュクエリを `20260622-1` に更新した。
- `public/sanga202627season.html` の `assets/app.js` クエリ、Version表記、Data updated表記を更新した。
- `docs/sheets/spreadsheet-export-check.md` に2026-06-22時点で49件へ反映した記録を追記した。
...
- `node --check public/assets/app.js` に成功した。
- `git diff --check` に成功した。
- NodeワンライナーでCSV/JSONとも49件、`sec01`〜`sec49` 欠落なし、ID重複なし、表示順1〜49欠番なし、大会コード `J1` / `EMP` / `LEV`、ホームアウェイ表示が `ホーム` / `アウェイ` / 空欄のみであることを確認した。
- `sec39` は `2026-08-26`、キックオフ時刻 `19:00` として出力されることを確認した。
- `sec45` は `2026-10-03|2026-10-04`、`sec48` / `sec49` は `2027-03-24` / `2027-03-28` として出力されることを確認した。

### 未確認項目

- 実ブラウザでのPC幅・スマートフォン幅の目視確認は未実施。今回の変更はデータ更新とJSONプレビューの空欄HOME/AWAY対応のみで、CSSは変更していない。
- GoogleスプレッドシートAPI / GAS連携は今回の対象外のため未実施。

### 残課題

- ルヴァン杯決勝は日付未定のため未追加。
- カップ戦の勝ち上がり後にホーム/アウェイ、対戦相手、会場、キックオフ時刻が確定したらCSV正本を更新し、同じ生成手順で再反映する。

### 人間が確認すべき点

- スプレッドシート正本と `docs/sheets/schedule.initial.csv` のカップ戦行が一致していること。
- 実ブラウザでカップ戦追加後の一覧表示、フィルタ、共有画像プレビューに大きな崩れがないこと。

## 2026-06-22 CI検証期待件数49件への更新

### 作業テーマ

カップ戦追加により `public/data/matches.json` が49件になったため、CIおよび現行ドキュメントの公開JSON検証コマンドに残っていた `--expected-count 38` を49件へ更新した。
...
- `docs/sheets/schedule-columns.md`
- `docs/sheets/schedule-template.md`
- `docs/ai/JS_CHANGE_CHECKLIST.md`
- `docs/ai/PLAN.md`
- `docs/ai/WORKLOG.md`
...
- Static Checksワークフロー内の `node tools/validate-generated-matches.js public/data/matches.json --expected-count 38 --strict` を `--expected-count 49 --strict` に更新した。
- 現在の公開JSONや本番用生成JSONを検証する手順・チェックリストの期待件数を49件へ更新した。
- 過去の作業履歴として38件だったことを示す既存WORKLOG等の記録は、履歴説明として残した。
...
- GitHub Actions上の実行結果はPR作成後に確認が必要。
