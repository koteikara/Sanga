...
CSSとJavaScriptの参照には、必ずバージョンクエリ（`?v=20260826-2` の形）を付けます。**付け忘れると、中身を直しても再訪した人には古いファイルが使われ続けます。**

* 中身を変えたファイルは、参照側の `?v=` も一緒に上げる。
* 変えていないファイルの版数は上げない。上げると全員に無駄な再ダウンロードが起きる。
* `tools/check-static-assets.mjs` が、公開HTMLの `.css` / `.js` / `.mjs` 参照すべてに `?v=` が付いているかを確認します。付いていないと `npm run check:static` が落ちます。付いているかどうかだけを見るので、**上げ忘れは検出できません。**

実際に `public/assets/app.js` からCDN依存を外したとき、参照側の版数を上げ忘れていました。そのまま反映していたら、再訪した人には古い（CDNを読む）JavaScriptが使われ続けるところでした。
