...
| 複数セッションで作業しているとき | 流す前に、他のセッションが動いていないか確かめる |
| 反映後にJavaScriptまで確かめる | 反映後にブラウザで確かめる |
...
CSSとJavaScriptの参照には、必ずバージョンクエリ（`?v=a21e3b0e` の形）を付けます。**付け忘れると、中身を直しても再訪した人には古いファイルが使われ続けます。**

版数は手で決めません。`tools/asset-versions.mjs` が参照先ファイルの内容のSHA-256（先頭8桁）を書き込みます。内容から決まるため、**中身を変えれば必ず版数が変わり、変えていないファイルの版数は変わりません。**

* CSS・JavaScriptを変更したら `npm run fix:asset-versions` を実行し、書き換わった参照も一緒にコミットする。
* `npm run check:static` に含まれる `tools/check-static-assets.mjs` が、参照側に書かれた `?v=` と実ファイルの内容ハッシュを突き合わせます。**上げ忘れはここで必ず落ちます。**
* 対象は公開HTMLの `href` / `src` だけでなく、`public/assets/` 配下のJavaScriptが静的importで読む相対パスも含みます（HTML側のクエリはimport先まで効かないため）。`public/assets/vendor/` は第三者のコードなので書き換えず、参照先としてだけ扱います。
* 参照が入れ子になっている場合（`index-nebula.js` が `index-motion.js` を読むなど）は、内側の版数が変わると外側の内容も変わるため、変化がなくなるまで繰り返して揃えます。

実際に `public/assets/app.js` からCDN依存を外したとき、参照側の版数を上げ忘れていました。そのまま反映していたら、再訪した人には古い（CDNを読む）JavaScriptが使われ続けるところでした。日付と連番を手で書いていたころは、この上げ忘れを検証で止められませんでした。
...
本番のファイルをそのまま手元に落とし、`localhost` で配ってブラウザに読ませます。
**本番と同じバイト列を、JavaScript込みで動かします。** 読み込み失敗と実行時エラーを拾います。

なぜ回り道をするのか。**この作業環境のChromiumは、外部のHTTPSへ出られません。**
`curl` も Node の `fetch` も通るのに、Chromiumだけが握手の途中で切られます
（外向きプロキシへのトンネルが落ちる）。原因は基盤側にあり、こちらでは直せません。
ポスト量子鍵交換・QUIC・HTTP/2を切る、TLS1.2を上限にする、証明書エラーを無視する、
どれも効きませんでした。2026-09-09に確認しています。

`curl` で本文を見るだけでは、JavaScriptが動くかどうかは分かりません。
落ちていても200は返ります。だからブラウザに読ませます。

**見られないものもあります。** 本番サーバーが返すヘッダ（MIMEタイプ・リダイレクト・
キャッシュ）、HTTPSそのもの、実オリジンに依存する挙動。**同じファイル、違う置き場所**
なので、そこは別物です。**実機での確認を置き換えるものではありません。**

既定では時系列と案内ページを見ます。ページ名を並べれば対象を変えられます。

```bash
node tools/check-production-pages.mjs squad.html calendar.html
node tools/check-production-pages.mjs --base https://koteikara.github.io/Sanga  # Pagesを見る
node tools/check-production-pages.mjs --keep    # 落としたファイルを残す
```

本番へ取りに行くので `npm run check` には入れていません。反映後に手で呼びます。
...
### 流す前に、他のセッションが動いていないか確かめる

複数のセッションで並行して作業していると、**本番デプロイが重なります。**
2026-09-08 に実際に重なりました。11秒差で2本が並び、片方を取り下げています。

ワークフローは `concurrency: production-deploy` / `cancel-in-progress: false` なので、
壊れはしません。順番待ちになるだけです。ただし2本目は、**同じ中身に対して暗号化退避と
FTP同期をもう一度やり直します。** 意味のない往復で、その間ずっと本番に触っています。

もっと困るのは、どちらの実行が誰のものか分からなくなることです。両方のセッションが
同じ実行を自分のものだと思い込み、片方は確認したつもりで誰も確認していない、
という抜けが起きます。実際に起きかけました。

流す前に、次を確かめます。

1. **`main` の先頭が、自分が最後にマージしたコミットか。**
   先にほかのコミットが乗っていたら、直前まで別のセッションが作業しています。
   ```bash
   git fetch origin main && git log --oneline origin/main -5
   ```
2. **デプロイのワークフローが走っていないか。** `queued` や `in_progress` の実行があれば、
   それが終わるまで待ちます。GitHubの `Actions` → `本番サーバー手動デプロイ` で見えます。
3. **自分の作業ぶんより先のコミットがあるなら、利用者に一声かける。**
   自分の変更だけを流すことはできません。**`main` の先頭がまるごと出ます。**
   他のセッションの変更も一緒に本番へ行くので、それでよいかを確かめます。

先が乗っていたときに伝えることは3つです。いま流すと何が一緒に出るか、
その部分を自分は確認していないこと、どちらのセッションが流すか決めてほしいこと。

**取り下げるときは、走っている方に任せます。** 同じコミットなら結果は変わりません。
先行しているものが失敗したときだけ、改めて起動します。

### 手動実行手順

1. シークレット `BACKUP_PASSPHRASE` が登録されていることを確認する。退避はWorkflowが自動で行う。
2. **他のセッションが動いていないか確かめる**（前節）。`main` の先頭と、走っているデプロイの有無を見る。
3. デプロイ対象の `main` のcommit SHAを確認し、記録する。**このSHAは、あとで「どの実行が自分のものか」を照合する手がかりになる。**
4. GitHubで対象リポジトリを開く。
5. `Actions` タブを開く。
6. `本番サーバー手動デプロイ` ワークフローを選ぶ。
7. `Run workflow` を選ぶ。
8. 対象ブランチに `main` を選ぶ。
9. `confirm` に `DEPLOY` と入力する。
10. `Run workflow` を実行し、実行URLを記録・共有する。
11. **一覧に自分の実行が1本だけであることを確認する。** 同じ時刻に2本並んでいたら、
    どちらが自分のものか（作成時刻で照合する）を確かめ、重複していれば後発を取り下げる。
12. 確認入力、スカッド事前検証、日程JSON・JavaScript検証、FTPアップロードがすべて成功したことを確認する。
13. `npm run check:production` を実行し、JavaScriptが動くことを確かめる（前節）。
14. 本番URLで表示と操作を確認する。**自分が変更していない部分も一緒に出ているので、
    そこを誰が確認するかをはっきりさせる。**
...
### `制御接続がタイムアウトしました。` / `Timeout (control socket)`

**接続そのものが立たない**症状です。押し直せば通ります。

2026-09-16までのデプロイ12回のうち4回が最初の1回で落ち、4回とも再実行で通りました。
落ちる場所は毎回違います（退避3回・アップロード1回）。特定の手順の問題ではなく、
スターレンタルサーバーへのFTP接続が不安定だということです。

原因候補:

* 相手サーバーの一時的な混雑または接続数の制限。
* GitHub Actionsのランナーから相手までの経路の問題。

対応（2026-09-17に自動化済み。ふつうは人が押し直す必要はありません）:

* **退避**（`tools/fetch-production-files.mjs`）は、回線側の失敗なら10秒・20秒空けて最大3回つなぎ直します。
  木を途中から再開できないので、**毎回まっさらからやり直します**（退避は2分ほどなので損は小さい）。
* **アップロード**（`FTP-Deploy-Action`）は他社のActionで中へ手が入らないため、
  同じ手順をワークフローにもう一度置いています。1回目に `continue-on-error` を付け、
  落ちたときだけ10秒待って2回目を実行します。2回目が落ちればジョブも落とします。

**つなぎ直すのは回線側の失敗だけです。** FTPコマンドが拒否された場合（`530 Login incorrect` など）は
1回で止めます。ログインの失敗を繰り返すと、相手のサーバーで締め出される恐れがあるためです。

3回とも落ちる場合は一過性ではありません。Secretsの値と、サーバー側の障害情報を確認してください。
