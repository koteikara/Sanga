> [!NOTE]
> この文書はPR #146・#151で実装済みとなった計画と検証記録です。現行の作業指示ではありません。
> 移動元: `docs/squad-bench-pitch-overlap-plan.md`
> アーカイブ日: 2026-08-21
...
作成日: 2026-08-20
状態: 段階1〜4 実装済み（段階1〜3: PR #146、段階4: PR #151）
...
- カード高さは**下限 70px の可変**とする。フォーメーション間の完全統一はやめる。
  ただし下限は**重なり回避の幾何的な上限を超えない範囲でのみ効かせる**。
  実測の結果、縦の列が5〜6段あるフォーメーション（4-2-3-1、4-1-4-1 など）では
  70px は物理的に到達できない（§6-2 の実測値）。上限を無視して下限を優先すると
  重なりが再発するため、下限より重なり回避を優先する。
...
### 段階 1: カード高さの下限を 70px の可変に変える（実装済み）
...
### 段階 2: ピッチの取り分を先に確保する（CSS のみ・実装済み）
...
### 段階 3: ベンチをキャンバス基準にし、枚数連動で縮める（実装済み）
...
### 段階 4: メンバー登録とピッチ配置を分離する（構造改善・実装済み）
...
| ファイル | 変更 |
|---|---|
| `public/assets/squad-builder.js` | `CARD_H_MIN=70` / `CARD_H_MAX=75.1` を定数化。`fitCards()` は `{ cardH, idealH, clamped }` を返す。`layoutPitch()` がベンチ縮小のフィードバックを1〜2回まわす。`renderBench()` を表示用と編集用に分離 |
| `public/assets/squad.css` | `.pitch{flex:1 1 auto;min-height:60%}` / `.sq-footer{flex:0 1 auto;max-height:24%;overflow:hidden}`。`.bench-slot` を cqw ＋ `--bench-scale` に。`.bench-editor` / `.bench-edit-slot` を追加 |
| `public/assets/squad-formations.js` | 密なフォーメーションのスロット縦間隔を広げた（§6-3） |
| `public/squad.html` | 設定パネルに「ベンチ（控えメンバー）」セクションを追加。バージョンクエリを更新 |
| `tools/check-squad-layout.mjs` | §5 の受け入れ条件を実ブラウザで検証するスクリプト（新規） |

計画からの差分:

- `layoutAll()` は新設せず、既存の `layoutPitch()` にフィードバックを内包した。
  呼び出し側（6箇所）を変えずに済むため。**契約は `layoutPitch()` が入口**となる。
- 出力時に空き枠を隠す `body.exporting` は不要になった。キャンバス内のベンチが
  表示専用になり、そもそも空き枠を描画しないため。
- ベンチ編集UIをキャンバス外へ出したのは段階4の一部だが、`.bench-slot` から
  `min-height:44px` を外すために先行して実施した。**状態モデル（`state.bench` /
  `BENCH_SIZE`）は変更していない**ので、段階4は引き続き必要。

### 6-2. 実測値（重要）

`tools/check-squad-layout.mjs` による実測。`pairBound`（重ならない上限）は
**ベンチではなくフォーメーションのスロット間隔で決まる**ことが分かった。

| キャンバス幅 | 4-4-2 | 4-2-3-1 | 4-1-4-1 |
|---|---|---|---|
| 296px（画面320px） | 59.5 | 44.8 | 40.0 |
| 351px（画面375px） | 72.2 | 54.6 | 48.5 |
| 396px（画面420px） | 82.1 | 62.3 | 55.3 |
| 420px（上限） | 87.1 | 66.1 | 59.1 |

- **4-2-3-1 と 4-1-4-1 は、どの幅でも 70px に届かない。** 縦に5〜6段あるため、
  段数 ×（カード高＋ピル高）がピッチ高を超える。物理的な制約であり、
  ベンチを縮めても解決しない。
- ポジションピルは 1枚あたり 11〜14px を消費する。ピルを非表示にすると
  4-2-3-1 は 420px 幅で 75.0px まで伸びる。
- したがって実装では、下限70pxは**幾何的上限を超えない範囲でのみ**適用している。
  4-4-2 / 4-3-3（4段）は 70〜75.1px、密なフォーメーションは 44〜66px になる。

**70px を全フォーメーションで満たしたい場合**の選択肢は次の3つだった。

1. `squad-formations.js` の `Y` 定数を見直し、密なフォーメーションの縦間隔を広げる → **採用（§6-3）**
2. ポジションピルをカードの下に積まず、カード下端に重ねる（1枚あたり 11〜14px 回収）
3. キャンバスの縦横比を 9:16 より縦長にする

### 6-3. スロット座標の見直し（設計変更・案1を採用）

§6-2 の制約を解消するため、密なフォーメーションの縦間隔を広げた。

**考え方**: 2枚のカードが重ならない条件は「横に十分離れている」か「縦に十分離れている」
かのどちらか。420px幅キャンバス（ピッチ 395×525px）で 70px を確保するには、
横が近い（dx < 14%）行どうしは **縦に 17.5% 以上**空ける必要がある。
...
`gk`(90) と `df`(70) と `mf`(44) は変更していない。

**個別に見直したフォーメーション**

| フォーメーション | 変更 |
|---|---|
| 4-4-1-1 | MF 44→46、FW 12→10 |
| 4-2-3-1 | FW 12→10 |
| 4-1-4-1 | 中盤 56→52、38→34、FW 13→11 |
| 4-3-1-2 | FW 13→11 |
| 4-1-2-1-2 | 中盤 56→52、30→28、FW 13→11 |

**結果（420px幅キャンバス・ベンチ9枠）**
...
| 4-1-2-1-2 | 55.3 | **71.8** |
| 3-4-2-1 | 70.5 | 70.5 |
| 4-4-2 / 4-3-3 ほか | 75.1 | 75.1 |

**全17フォーメーションが 70.5〜75.1px に収まり、重なりは0組。**

補足: 画面幅375px では 62.0〜71.8px、320px では 51.3〜59.5px になるが、これは
キャンバス自体が小さいため。**キャンバス幅に対する比率は同じ（約18.1%）**なので、
出力画像の見え方は変わらない。下限70pxは「420px幅キャンバスでの絶対値」として
満たしている。

### 6-4. 検証結果

`tools/check-squad-layout.mjs`（全17フォーメーション × 幅320/375/420px × ベンチ0/5/9枠 = 153通り）

| | 修正前 | 修正後 |
|---|---|---|
| カードの重なり | 2〜6組（全条件で発生） | **0組** |
| ピッチからのはみ出し | 0 | 0 |
| ベンチの見切れ | なし | なし |
| フッターの占有率 | 19.0〜20.6% | **11.2〜12.4%**（ベンチ枠数にほぼ非依存） |
| ピッチの占有率 | 60.8〜62.3% | **69.0〜70.3%** |

未確認:

- 実機（iOS / Android）での目視確認
- PNG出力（`modern-screenshot`）経由での最終画像の目視確認。
  ヘッドレスChromiumでのDOM実測とスクリーンショットまでは確認済み。


### 6-4. 段階4の実装内容（2026-08-20）

状態モデルを次のように変えた。

```
state.squad  登録メンバー（背番号の配列・人数自由）
state.slots  11スロット。playerNumber は squad の要素を指す
控え         squad のうちピッチに置かれていない人（getBench() で導出）
```

- `state.bench` と `BENCH_SIZE`（`squad-formations.js`）を廃止した。ベンチ人数の上限はない
- ピッチに選手を置くと自動的に登録される。ピッチの枠を空にすると登録からも外れる
- 控えの編集UI（追加・削除）はキャンバス外にあり、タップ領域は44pxを確保している
  （×ボタンは見た目38pxだが、擬似要素で当たり判定を44pxに広げている）
- 保存は新形式（`squad`）のみ。読み込みは旧形式（`bench` の9枠固定配列）も受け付け、
  `slots` の背番号と合わせて `squad` に畳み込む。背番号は文字列に正規化する
- **旧形式での書き戻しは行わない**（合意済み）。新形式で保存した後は、
  旧バージョンのページでは控えが復元されない

検証: 全17フォーメーション × 幅320/375/420px × 控え0/5/9/12人 = 204通りで
重なり・はみ出し・ベンチの見切れがないこと（`tools/check-squad-layout.mjs`）。
控え12人でもフッター占有率は11.2%、カード高さは420px幅で70.5〜75.1pxを維持。
...
| リリース | 含む段階 | 効果 | 状態 |
|---|---|---|---|
| R1 | 段階 1 | 重なりの大半が解消 | 済（PR #146 にまとめて実装） |
| R2 | 段階 2 ＋ 段階 3 | 重なり完全解消・ベンチ欠けなし（この2つは分割しない） | 済（PR #146） |
| R3 | 段階 4 | 構造改善・ベンチ人数自由化 | 済（PR #151） |
...
> **現況（2026-08-20）**: 段階1〜4 すべて実装済み（担当A〜Dの範囲は完了）。
> 残るは担当Eによる実機確認のみ。以下は当初の分割案で、記録として残す。

複数の実装者（またはエージェント）で並行実装する場合の分割案。
**編集ファイルが重ならないこと**を第一の基準に切っている。

### 8-0. 先に入れる「契約コミット」C0（必須・並行作業の前提）
