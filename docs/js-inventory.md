...
- `STORAGE_KEYS` でLocalStorageキーを一覧化し、既存契約として参照される `CARD_STATE_STORAGE_KEY`、`FILTER_SETTINGS_STORAGE_KEY`、`DISPLAY_MODE_STORAGE_KEY`、`LAYOUT_STORAGE_KEY` は維持しています。
- 汎用の `readStorageValue()`、`writeStorageValue()`、`removeStorageValue()` に加え、JSON保存用の `readStorageJson()`、`writeStorageJson()` を使い、保存・復元処理の責務を分けています。
- 試合カード状態、フィルタ状態、表示列、表示モードの既存キーと保存形式は変更していません。
- 保存内容削除は `STORAGE_CLEAR_KEYS` を使って4キーを削除し、削除後はタップ状態なし / 2列 / 通常カード / すべて表示へ戻します。
- 今回は新しいLocalStorageキー追加や保存形式変更は行っていません。
