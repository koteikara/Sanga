...
| `public/squad.html` | 設定パネルに「ベンチ（控えメンバー）」セクションを追加。バージョンクエリを `v=20260820-5` へ |
...
**70px を全フォーメーションで満たしたい場合**は、次のいずれかの設計変更が要る。
いずれも見た目が変わるため、実施は別途判断が必要。

1. `squad-formations.js` の `Y` 定数を見直し、密なフォーメーションの縦間隔を広げる
