...
### 段階 4: メンバー登録とピッチ配置を分離する（構造改善・未着手）
...
> **現況（2026-08-20）**: 担当A・B・C（段階1〜3）は PR #146 で実装済み。
> 残るは**担当D（段階4：状態モデルの分離）**と、担当Eによる実機確認。
> 以下は当初の分割案で、段階4を進める際の参照として残す。
