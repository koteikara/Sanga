...
| タイムゾーン | `Asia/Tokyo`。日本の利用者が見る画面を撮る |
...
以前は `public/assets/app.js` が CDN からモジュールを読んでおり、CDNへ到達できない環境では日程表が一切描画されませんでした。リポジトリ内の静的配置へ切り替えたため、いまは差し替えなしで撮影できます。

## OGP画像の作り直し

`public/assets/ogp.jpg` は `experiments/site-index/ogp-source.html` を1200×630で描画して書き出したものです。文言や見た目を変えたら、元のHTMLを直して撮り直します。

| 項目 | 値 |
| --- | --- |
| 表示 | 1200×630、画素密度1倍 |
| 出力 | JPEG 品質88（PNGだと452KB、JPEGで53KB） |

クラブの紋章やマークは使いません。このサイトのキャラクターと文字だけで作ります。

撮り直したら `node tools/asset-versions.mjs` を実行してください。画像の名前は同じままなので、版数を上げないと再訪した人には古い画像が使われ続けます。
