...
プロンプトでは越えられないので、**Codex のトップからリポジトリを選んで新規タスクとして
開始し直します**（環境やフォルダから入らない）。
...
```text
PRが作れない場合でも git reset は絶対にしないこと。
コミットを残したまま、git diff HEAD~1 の全文を出力して終わること。
```

作業の終わりに、コミットが積まれたことも確かめさせます。
...
プラットフォーム側もPRを作れないので、ボタンが出ないのは正しい挙動です。
...
* PRが作れないとき、`git reset --hard` などで作業を捨てない。コミットを残して差分を出力する。
