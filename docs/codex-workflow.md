...
すべての実装者は `AGENTS.md`、`docs/documentation-policy.md`、`docs/project-structure.md` と、変更対象別の仕様・手順・チェックリストを読みます。実装や運用を変更した場合は同じPRで現行文書を更新し、不要な場合も理由をPR本文に記載します。
...
* UIを伴う新機能や大きな見た目変更では、先に `docs/ui-prototype-workflow.md` を確認し、`experiments/` でプロトタイプを作成してから本番実装に進みます。
...
PR作成前に、PR本文の見出しが次の4つだけになっているか確認します。
...
また、PR本文に次の英語見出しが含まれていないことを確認します。
