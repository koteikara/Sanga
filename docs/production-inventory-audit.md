...
**サーバー上の実物はデプロイでは消えません。** 後述の「FTP同期が削除できる範囲」のとおり、同期の記録に無いファイルはActionの対象外です。サーバー側は手動削除が必要で、2026-08-26時点では未実施です。
...
当初はデプロイの `exclude` で除外しましたが、除外したファイルは同期の比較対象から外れるだけで、サーバー上からは削除されません。そこで `public/experiments/` 自体を削除し、リポジトリ直下の `experiments/` を唯一の正本にします。これらは同期の記録にあるため、次回のデプロイでサーバーからも削除されます。

副次的に、`public/experiments/` とルート `experiments/` の二重管理も解消します。
...
この取り込み作業により、`images/` を除く51件が同期管理下に入りました。以降はリポジトリ側の削除がサーバーへ反映されます。一方、`images/` はリポジトリから削除済みでサーバーへ送られないため、同期管理下に入る機会がありません。**サーバー上の `images/` は手動で削除する必要があります。**

`exclude` に入れたファイルも同じく比較対象から外れます。公開対象から外したいものは、除外ではなく `public/` に置かないことで対応します。

## 未解決の課題

### サーバー上の `images/` が未削除

`STAR_SERVER_REMOTE_DIR` 配下の `images/`（39件・528KB）は、同期管理外のためデプロイでは削除されません。FTPクライアントでディレクトリごと手動削除してください。2026-08-26時点で未実施です。
