/** extract_candidates.gs */

function extractCandidates_(title, bodyText, article, publishedDate) {
  var items = [];
...
function extractFromTitle_(title, article, publishedDate) {
  ...
  while ((m = reRange.exec(title)) !== null) {
    ...
    var typ = guessTypeFromContext_(title, "title");
    out.push({
      type: typ,
      label: "title-range",
      start: start,
  ...
  while ((m = reDeadline.exec(title)) !== null) {
    ...
    var typ2 = guessTypeFromContext_(title, "title");
    out.push({
      type: typ2,
...
function extractFromBody_(text, article, publishedDate) {
  ...
  while ((m = reRange2.exec(text)) !== null) {
    ...
    var y1 = resolveYearForMonth_(year, sm, publishedDate);
    var y2 = resolveYearForMonth_(year, em, publishedDate);
    var typ2 = guessTypeFromContext_(ctx2, "body");
  ...
  while ((m = reDeadline.exec(text)) !== null) {
    ...
    var typ3 = guessTypeFromContext_(ctx3, "body");
    out.push({
      type: typ3,
