 ...
 *
 * 読むデータ
 *   public/data/matches.json（正本をそのまま読む。サンプルの複製は置かない）
 *
 * 間隔の決め方
...
(function () {
  ...
  // 日程JSONは公開物の正本を読む。GitHub Pagesでは public/ が直下へ、
  // 手元の `python3 -m http.server` ではリポジトリ直下が根になるため、両方を試す。
  var DATA_URLS = ["../../data/matches.json", "../../public/data/matches.json"];
  ...
  function loadMatches() {
    var attempt = 0;
    function next() {
      if (attempt >= DATA_URLS.length) {
        return Promise.reject(new Error("matches.json を読めませんでした"));
      }
  ...
  loadMatches().then(function (data) {
    ...
    statusLine.textContent = "日程を読み込めませんでした。HTTP経由で開いているか確認してください。";
    strip.replaceChildren(statusLine);
    console.warn("dense-schedule-calendar:", error);
  });
})();
