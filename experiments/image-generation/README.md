...
8. 「画像を保存」リンクとスマホ向けの長押し保存案内が表示されることを確認します。
...
* PCでは画像保存リンクを使えるか
* PC Chromeで動くか
* iPhone Safariでプレビュー画像を長押し保存しやすいか
...
* `html-to-image`: 画像生成成功
* `modern-screenshot`: 画像生成成功
...
* `modern-screenshot`: 画像生成成功
* 生成画像の見た目は概ね問題なし
* 保存導線は「ダウンロード」より「画像を保存」または長押し保存案内が自然

実装上は、画像化対象を `#share-capture-target` に分離することで、列切替UI、画像生成UI、画像保存リンク、検証用メッセージ、hidden要素を対象外にできます。
